...
namespace Planetarium.Deployables
{
    ...
    public class DeployableBase : MonoBehaviour, IDamageable, ITaggable
    {
        ...
        protected TaggedComponent taggedComponent;

        // Cached tags for better performance
        private static class CachedTags
        {
            public static readonly GameplayTag DeployableBase = new GameplayTag("Deployable.Base");
            public static readonly GameplayTag DeployableActive = new GameplayTag("Deployable.Active");
            public static readonly GameplayTag DeployableTurret = new GameplayTag("Deployable.Turret");
        }

        protected virtual void Awake()
        {
            taggedComponent = GetComponent<TaggedComponent>();
            taggedComponent.AddTag(CachedTags.DeployableBase);
            taggedComponent.AddTag(CachedTags.DeployableTurret);
        ...
        protected virtual void Start()
        {
           ...
           // TaggedStatsHelper.OnTurretBuilt(M_TurretStats.GetName());
        }
        ...
        protected virtual void Die()
        {
           ...
           // TaggedStatsHelper.OnTurretSold(M_TurretStats.GetName());
        ...
        public virtual void OnTagAdded(GameplayTag tag)
        {
            // Handle tag added
            if (tag.Matches(CachedTags.DeployableActive))
            {
                OnActivated();
            }
        }

        public virtual void OnTagRemoved(GameplayTag tag)
        {
            // Handle tag removed
            if (tag.Matches(CachedTags.DeployableActive))
            {
        ...
        {
            return taggedComponent.HasTag(CachedTags.DeployableActive);
        }

        public void Activate()
        {
            taggedComponent.AddTag(CachedTags.DeployableActive);
        }

        public void Deactivate()
        {
            taggedComponent.RemoveTag(CachedTags.DeployableActive);
        }
        ...
        protected virtual void FireProjectile(Vector3 targetPos)
        {
            ...
            {
                var projectile = Instantiate(M_Projectile, TurretMuzzle.position, Quaternion.identity);
                projectile.Initialize(M_TurretStats.GetDamage(), targetPos, M_TurretStats.GetProjectileSpeed());
                projectile.SetSource(ProjectileSource.Turret, M_TurretStats.GetName());
                projectile.ShootProjectile(targetPos, ClosestTarget.gameObject);
            }
        ...
        {
            if (M_Projectile != null && target != null)
            {
                var projectile = Instantiate(M_Projectile, TurretMuzzle.position, Quaternion.identity);
                if (projectile != null)
                {
